...
export class ServerRepository {
  ...
  list(): ServerRecord[] {
    const rows = this.db.prepare('SELECT * FROM servers ORDER BY name COLLATE NOCASE').all() as unknown as ServerRow[];
    return rows.map(mapRow);
  }

  findById(id: number): ServerRecord {
    const row = this.db.prepare('SELECT * FROM servers WHERE id = ?').get(id) as ServerRow | undefined;
    if (!row) throw new NotFoundError('Server', String(id));
    return mapRow(row);
  }
  ...
  findByName(name: string): ServerRecord {
    ...
    if (!row) throw new NotFoundError('Server', name);
    return mapRow(row);
  }
  ...
  update(id: number, input: Partial<CreateServerInput>): ServerRecord {
    ...
    `).run(
      ...
      next.username,
      next.authType,
      next.encryptedPassword ?? null,
      next.encryptedPrivateKey ?? null,
      next.encryptedPassphrase ?? null,
      next.tags ?? null,
      next.notes ?? null,
      next.defaultRemotePath ?? null,
      now,
      id
    );
